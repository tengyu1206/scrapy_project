...
class Fang_newhouse_Spider(scrapy.Spider):
            ...
            if count > 1:            
                ...
                for cityinfo in cityInfoList:
                    ...
                    for url in (onsellUrl,waitsale):           
                        ...
                        time.sleep(6)
                    '''
                    request1 = scrapy.Request(waitsale,headers=self.headers,callback=self.parse_fanglist_data)
    ...
    def parse_fanglist_data(self, response):
        ...
        #print next_page
        if(len(next_page) != 0):
            next_suffix = next_page[0]["href"].strip().split("/")[-2]
            last = url.strip().split("/")[-2]
            next_url =  url.replace(last, next_suffix)
            #print "next_url:",next_url
            request = scrapy.Request(next_url,headers=self.headers,callback=self.parse_fanglist_data)
            request.meta['cityName'] = response.meta['cityName']
            #request.meta['state'] = response.meta['state']
            yield request
            time.sleep(4) 
    ...
    def parse_selfsale_data(self, response):    
        #print "selfsale_url:",response.url      
        html = response.body
        soup = BeautifulSoup(html, 'html5lib')
        houseList = soup.select('div.nl_con.clearfix > ul > li')
